...
document.addEventListener('keydown', (event) => {
  if (event.ctrlKey || event.metaKey || event.altKey || event.shiftKey) {
    if (
      ...
      (event.ctrlKey && event.key.toLowerCase() === 'y' && event.shiftKey)
    ) {
      event.preventDefault();
      event.stopPropagation();
    }
  ...
  if (event.ctrlKey || event.metaKey) {
    if (
      ...
      (event.key === 'F' && event.shiftKey) ||
      (event.key === 'L' && event.shiftKey)
    ) {
      event.preventDefault();
      event.stopPropagation();
    }
  ...
  const check = () => {
    if (
      ...
      if (!devtoolsOpen) {
        ...
        setTimeout(() => {
          window.close();
          location.href = "about:blank"; // fallback nếu không đóng được tab
        }, 100); // 0.1 giây
      }
    } else {
      devtoolsOpen = false;
